...
class UserTransactionsTable extends StatefulWidget {
  const UserTransactionsTable({required this.rows, super.key});

  final List<PaymentTransaction> rows;

  @override
  State<UserTransactionsTable> createState() => _UserTransactionsTableState();
}

class _UserTransactionsTableState extends State<UserTransactionsTable> {
  String _query = '';
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _PaymentsFilterBar(
          searchHint: 'Search user transactions',
          status: _status,
            ...
            rows: [
              for (final row in filteredRows)
                [
                  ...
                  _ViewCell(onPressed: () => _showDetails(context, row)),
                ],
            ],
          ),
        ),
    ...
    return SingleChildScrollView(
      child: SingleChildScrollView(
        ...
        child: SizedBox(
          ...
          child: Column(
            children: [
              _RowShell(isHeader: true, children: header),
              if (rows.isEmpty)
                Padding(
                ...
                for (final row in rows) _RowShell(children: row),
            ],
          ),
        ),
      ),
